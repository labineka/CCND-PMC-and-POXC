...
#To create the standard curve for each plate, determine the slope (b) and 
#y-intercept (a) of a linear regression equation with concentration as the dependent variable (y) and absorbance as the independent variable (x)
...
sample_mass <- read_csv("raw_data/poxc.soil.mass.25.csv") %>% 
  ...
  select(-c(mass_g)) %>% 
  rename(id = sample_id)
names <- read_csv("raw_data/CCNRD_sample names.csv") %>% 
  rename(id = sample_id)
...
write_csv(avg_poxc_calc, glue("./results/poxc_calculations_avg_{today}.csv"))

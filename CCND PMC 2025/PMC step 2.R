...
mergeAll=rbind(Run2,Run3,Run4,Run5,Run6,Run7)
#,,Run4,Run5,Run6,Run7,Run8
head(mergeAll)
mergeClean=select(mergeAll,plot,depth.cm,sample.id,cup_label,ugC.g.day)


write.csv(mergeClean, "PMC_CCND_all_calcs_29Dec25.csv",
          row.names = FALSE)
...
all.data<-read.csv("PMC_CCND_all_calcs_29Dec25.csv", stringsAsFactors = FALSE) 
...
write.csv(pmc.var, "PMC_CCND_all_calcs_CV_29Dec25.csv")
